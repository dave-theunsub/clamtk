...
sub show_window {
    ...
    my $pixbuf =
      Gtk3::Gdk::Pixbuf->new_from_file_at_size( "$images_dir/clamtk.png", 24,
        24 );
    ...
    my $label =
      Gtk3::Label->new( _('Select a time to scan your home directory') );
    $scan_box->pack_start( $label, FALSE, FALSE, 5 );

    $label = Gtk3::Label->new( _('Set the scan time using a 24 hour clock') );
    $scan_box->pack_start( $label, FALSE, FALSE, 5 );
    ...
    $label = Gtk3::Label->new( _('Select a time to update your signatures') );
...
sub apply_defs {
    ...
    if (
        ClamTk::Prefs->get_preference('Update') eq 'single'
